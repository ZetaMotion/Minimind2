...
//    }
    ...
    func sum(_ arr: [Element]) -> Element {
...
public func sum<T: SignedInteger>(_ arr: [T]) -> T {
    return arr.reduce(0, {x,y in x + y})
}

public func prod<T: SignedInteger>(_ arr: [T]) -> T {
    return arr.reduce(0, {x,y in x * y})
}

public func mean<T: SignedInteger>(_ arr: [T]) -> Int {
