...
public struct Matrix<T> {
    ...
    public init(_ data: [[Element]]) {
        ...
        precondition(Minimind2.all(data.map{ $0.count } == data[0].count) , "all dimensions in data must be equal")
        rows = data.count
...
public extension Matrix where T: ScalarType {
    ...
    func apply(_ f: ([T]) -> T, _ axis: Int) -> Matrix {
        if axis == 0 {
    ...
    // apply a function to reduce an axis to scalar
    func apply<T2: ScalarType>(_ f: ([T]) -> T2, _ axis: Int) -> Matrix<T2> {
        if axis == 0 {
            var m: Matrix<T2> = Minimind2.zeros(1, columns)
            for col in 0..<columns {
                m[0, col] = f(self[column: col].grid)
            }
            return m
        } else if axis == 1 {
            var m: Matrix<T2> = Minimind2.zeros(rows, 1)
            for row in 0..<rows {
                m[row, 0] = f(self[row].grid)
            }
            return m
    ...
    func apply(_ f: ([T]) -> [T], _ axis: Int) -> Matrix<Element> {
    ...
    func apply(f: ([T], [T]) -> [T], arr: [T], axis: Int) -> Matrix<Element> {
        var re: Matrix<T> = self.zeros(rows, columns)
        switch axis {
        case 1: for c in 0..<columns {
    ...
    func zeros(_ rows: Int, _ columns: Int) -> Matrix {
        return Minimind2.zeros(rows, columns)
    }
}
...
//infix operator**
//public func **<T: ScalarType>(lhs: Matrix<T>, rhs: T) -> Matrix<T> {
//    var newmat = lhs
//    newmat.grid = lhs.grid ** rhs
//    return newmat
//}
...
infix operator |+
public func |+<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in add(x, y: y)}, arr: rhs.grid, axis: 1)
}

infix operator |-
public func |-<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in sub(x, y: y)}, arr: rhs.grid, axis: 1)
}

infix operator |*
public func |*<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in mul(x, y: y)}, arr: rhs.grid, axis: 1)
}
...
infix operator |/
public func |/<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in div(x , y: y) }, arr: rhs.grid, axis: 1)
}
...
infix operator .+
public func .+<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in add(x , y: y) }, arr: rhs.grid, axis: 0)
}

infix operator .-
public func .-<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in sub(x, y: y)}, arr: rhs.grid, axis: 0)
}

infix operator .*
public func .*<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in mul(x, y: y)}, arr: rhs.grid, axis: 0)
}
...
infix operator ./
public func ./<T: ScalarType>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
    return lhs.apply(f: {(x: [T], y: [T]) -> [T] in div(x , y: y)}, arr: rhs.grid, axis: 0)
}
...
public func abs<T: ScalarType>(_ mat: Matrix<T>) -> Matrix<T> {
    var newmat = mat
    newmat.grid = Minimind2.abs(newmat.grid)
    return newmat
}
